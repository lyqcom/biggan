...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        // std::cout << "Ready to trans file to Tensor..." << std::endl;
        auto image = ReadFileToTensor(all_files[i]);
        composeDecode(image, img.get());
        // std::cout << "Trans file to Tensor done" << std::endl;
        ...
        for (size_t k = 0; k < n_attrs; ++k) {
            ...
            // std::cout << "ready for model pred ..." << std::endl;
            Status gen_model_ret = gen_model.Predict(inputs, &outputs);
            // std::cout << "Get model_pred stu:" << gen_model_ret << std::endl;
