...
class Inception_A(nn.Cell):
    def __init__(self, in_channels, pool_features, has_bias=False):
        ...
        self.branch0 = BasicConv2d(
            in_channels, 64, kernel_size=1, has_bias=has_bias)
        ...
        self.branch_pool = nn.SequentialCell([
            ...
            BasicConv2d(in_channels, pool_features,
                        kernel_size=1, has_bias=has_bias)
        ])
...
class Inception_B(nn.Cell):
    def __init__(self, in_channels, has_bias=False):
        ...
        self.concat = P.Concat(axis=1)
        self.branch0 = BasicConv2d(
            in_channels, 384, kernel_size=3, stride=2, pad_mode='valid', has_bias=has_bias)
        self.branch1 = nn.SequentialCell([
            BasicConv2d(in_channels, 64, kernel_size=1, has_bias=has_bias),
            BasicConv2d(64, 96, kernel_size=3, has_bias=has_bias),
...
class Inception_C(nn.Cell):
    def __init__(self, in_channels, channels_7x7, has_bias=False):
        ...
        self.concat = P.Concat(axis=1)
        self.branch0 = BasicConv2d(
            in_channels, 192, kernel_size=1, has_bias=has_bias)
        self.branch1 = nn.SequentialCell([
            BasicConv2d(in_channels, channels_7x7,
                        kernel_size=1, has_bias=has_bias),
            BasicConv2d(channels_7x7, channels_7x7,
                        kernel_size=(1, 7), has_bias=has_bias),
            BasicConv2d(channels_7x7, 192, kernel_size=(
                7, 1), has_bias=has_bias)
        ])
        self.branch2 = nn.SequentialCell([
            BasicConv2d(in_channels, channels_7x7,
                        kernel_size=1, has_bias=has_bias),
            BasicConv2d(channels_7x7, channels_7x7,
                        kernel_size=(7, 1), has_bias=has_bias),
            BasicConv2d(channels_7x7, channels_7x7,
                        kernel_size=(1, 7), has_bias=has_bias),
            BasicConv2d(channels_7x7, channels_7x7,
                        kernel_size=(7, 1), has_bias=has_bias),
            BasicConv2d(channels_7x7, 192, kernel_size=(
                1, 7), has_bias=has_bias)
        ])
        self.branch_pool = nn.SequentialCell([
            nn.AvgPool2d(kernel_size=3, pad_mode='same'),
...
class Inception_E(nn.Cell):
    def __init__(self, in_channels, has_bias=False):
        ...
        self.concat = P.Concat(axis=1)
        self.branch0 = BasicConv2d(
            in_channels, 320, kernel_size=1, has_bias=has_bias)
        self.branch1 = BasicConv2d(
            in_channels, 384, kernel_size=1, has_bias=has_bias)
        self.branch1_a = BasicConv2d(
            384, 384, kernel_size=(1, 3), has_bias=has_bias)
        self.branch1_b = BasicConv2d(
            384, 384, kernel_size=(3, 1), has_bias=has_bias)
        self.branch2 = nn.SequentialCell([
            BasicConv2d(in_channels, 448, kernel_size=1, has_bias=has_bias),
            BasicConv2d(448, 384, kernel_size=3, has_bias=has_bias)
        ])
        self.branch2_a = BasicConv2d(
            384, 384, kernel_size=(1, 3), has_bias=has_bias)
        self.branch2_b = BasicConv2d(
            384, 384, kernel_size=(3, 1), has_bias=has_bias)
        self.branch_pool = nn.SequentialCell([
            nn.AvgPool2d(kernel_size=3, pad_mode='same'),
            BasicConv2d(in_channels, 192, kernel_size=1, has_bias=has_bias)
...
class InceptionV3(nn.Cell):
    def __init__(self, num_classes=10, is_training=True, has_bias=False, dropout_keep_prob=0.8, include_top=True):
        ...
        self.Conv2d_1a = BasicConv2d(
            3, 32, kernel_size=3, stride=2, pad_mode='valid', has_bias=has_bias)
        self.Conv2d_2a = BasicConv2d(
            32, 32, kernel_size=3, stride=1, pad_mode='valid', has_bias=has_bias)
        self.Conv2d_2b = BasicConv2d(
            32, 64, kernel_size=3, stride=1, has_bias=has_bias)
        self.maxpool1 = nn.MaxPool2d(kernel_size=3, stride=2)
        self.Conv2d_3b = BasicConv2d(64, 80, kernel_size=1, has_bias=has_bias)
        self.Conv2d_4a = BasicConv2d(
            80, 192, kernel_size=3, pad_mode='valid', has_bias=has_bias)
...
if __name__ == '__main__':
    ...
    input_x = mindspore.Tensor(numpy.random.rand(
        5, 3, 128, 128), mindspore.float32)
    out_ = my_model(input_x)
    print("get out shape", out_.shape)
